...
function ResourceForm({ onResourceCreated }) {
  ...
  const getCostPreview = () => {
    const { billing_type, usage_hours, cost_per_hour, monthly_cost, billing_period_months, request_count, cost_per_request, storage_gb, cost_per_gb_month } = formData;
    const hours = parseFloat(usage_hours || 0);
    const hourly = parseFloat(cost_per_hour || 0);
    const monthly = parseFloat(monthly_cost || 0);
    const months = parseInt(billing_period_months || 1, 10) || 1;
    const count = parseInt(request_count || 0, 10);
    const perRequest = parseFloat(cost_per_request || 0);
    const storage = parseFloat(storage_gb || 0);
    const storagePrice = parseFloat(cost_per_gb_month || 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.billing_type === 'hourly') {
        resourceData.usage_hours = parseFloat(formData.usage_hours);
        resourceData.cost_per_hour = parseFloat(formData.cost_per_hour);
      }
      if (formData.billing_type === 'monthly') {
        resourceData.monthly_cost = parseFloat(formData.monthly_cost);
        resourceData.billing_period_months = parseInt(formData.billing_period_months, 10);
      }
      if (formData.billing_type === 'per_request') {
        resourceData.request_count = parseInt(formData.request_count, 10);
        resourceData.cost_per_request = parseFloat(formData.cost_per_request);
      }
      if (formData.billing_type === 'storage_gb_month') {
        resourceData.storage_gb = parseFloat(formData.storage_gb);
        resourceData.cost_per_gb_month = parseFloat(formData.cost_per_gb_month);
      }
  ...
  return (
    <div className="resource-form-container">
      ...
      <form onSubmit={handleSubmit} className="resource-form">
        ...
        {success && <div className="form-success">✓ Resource created successfully!</div>}
        ...
        <div className="form-group">
          ...
          <input
            ...
            placeholder="e.g., Web Server 1, Database Cluster"
            required
          />
        </div>
